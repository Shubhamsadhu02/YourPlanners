    ...
    <section className="my-16 container" id="menu">
      <div className="w-full flex flex-col items-center justify-center">
        ...
        <div className="w-full flex flex-col items-center">
          ...
          <input
        type="text"
        placeholder="Search With Company Name, Area..."
        value={searchValue}
        onChange={handleSearch}
        className="border rounded my-8 p-3 w-80 lg:w-96 hover:border-indigo-500  text-sm md:text-base"
      />
      <AiOutlineSearch size={20} className="absolute font-bold right-2 bottom-12"/>
        ...
        .filter((n) =>n.company.toLowerCase().includes(searchValue.toLowerCase()) || n.area.toLowerCase().includes(searchValue.toLowerCase()))
        .map((item, index) => (
